...
# Twin Towers V9 properties (1:50 scale, units: kN, cm, sec)
set g         981.0        ;# cm/s2
set numFloors 25
set H_story   6.12         ;# cm per story (153/25)
set H_total   153.0        ;# cm total height

# Material: steel wire 6mm x 6mm section
set E_mat     170.0        ;# kN/cm2
set G_mat     65.385       ;# kN/cm2

# Per-floor lumped mass (total 40.62 kg distributed over 25+1 floors)
# From V9 model: ~0.6 kg/floor for tower floors, more at bridge floors
set massPerFloor 0.000813  ;# kN*s2/cm (= 0.798 kg / 981)
set totalMass    [expr $massPerFloor * $numFloors]

# Equivalent column properties per floor
# Calibrated stick model: EI_eff accounts for full frame action
# (columns + beams + braces + frame rigidity of 2 towers)
# For braced frames, EI_eff >> sum of column Iz
# Calibration target: T1 ~ 0.3-0.5 s (typical for 1:50 scale braced building)
# Formula: T1 = 36.98 / sqrt(Iz_eff) for this cantilever model
# T1=0.37s -> Iz=10000, T1=0.30s -> Iz=15000
set A_col    [expr 2.0 * 32 * 0.36]                ;# 2 towers * 32 cols * 0.36 cm2
set Iz_col   10000.0                                ;# Calibrated EI_eff for braced frame (cm4)
set Iy_col   $Iz_col
set J_col    [expr 2.0 * 32 * 0.0182 * 50.0]       ;# Torsional stiffness (amplified)
...
for {set i 1} {$i <= $numFloors} {incr i} {
    set m $massPerFloor
    # Bridge floors (6,7,12,13,18,19,25) get extra mass
    if {$i == 6 || $i == 7 || $i == 12 || $i == 13 || $i == 18 || $i == 19 || $i == 25} {
        set m [expr $massPerFloor * 1.3]  ;# 30% extra for sky bridge mass
    }
...
# Elements: elastic beam-column for each story
puts ""
...
for {set i 1} {$i <= $numFloors} {incr i} {
    set ni [expr $i - 1]
    set nj $i
    # Stiffness reduction for upper floors (typical frame behavior)
    set sf 1.0
    if {$i > 15} { set sf 0.80 }
    if {$i > 20} { set sf 0.65 }
...
# First mode shape (for pushover load distribution)
set modeShape {}
for {set i 1} {$i <= $numFloors} {incr i} {
...
puts "##  1999 Duzce Earthquake - Bolu Station (BOL090)            ##"
puts "################################################################"
puts ""
...
if {![file exists $gmFile]} {
    ...
    puts "*** Tried: ../../ground_motion/BOL090_scaled_1_50.AT2"
    puts "*** Tried: C:/Users/lenovo/Desktop/DASK_NEW/ground_motion/BOL090_scaled_1_50.AT2"
} else {
...
puts ">>> PGA = [format "%.6f" $pga] g = [format "%.4f" [expr $pga * $g]] cm/s2"
puts ""

# Write single-column file (convert g to cm/s2)
file mkdir ground_motion
set tempGM "ground_motion/BOL090_single_col.txt"
...
set xi 0.05
set omega3 [expr $omega1 * 3.0]  ;# approx 3rd mode
...
puts "  a0 (mass) = [format "%.6f" $a0]"
puts "  a1 (stiff) = [format "%.6f" $a1]"
puts ">>> Rayleigh damping applied"
puts ""

# ============================================================================
# RECORDERS FOR TIME HISTORY
# ============================================================================
puts "============================================================"
...
puts "  (OK) Roof displacement, velocity, acceleration"
puts "  (OK) Mid-height displacement (floor 13)"
puts "  (OK) Base reaction"
puts "  (OK) Envelope roof displacement"
puts "  (OK) All floor X-displacements"
puts "  (OK) Interstory drift at floors 5,10,15,20,25"
puts ""
...
set dt_a $dt_gm   ;# Use same dt as ground motion for speed
set Nsteps_TH [expr int($npts_gm)]
...
puts "  Numberer:     RCM"
puts "  System:       BandSPD"
puts "  Test:         NormDispIncr (tol=1e-6, maxIter=25)"
puts "  Algorithm:    Newton"
puts "  Integrator:   Newmark (0.5, 0.25)"
puts "  Analysis:     Transient"
puts ""
puts "  dt_analysis:  $dt_a s"
puts "  Total steps:  $Nsteps_TH"
puts "  Duration:     [format "%.1f" [expr $Nsteps_TH * $dt_a]] s"
puts ""
...
puts ""
puts "  Final Base Shear (X):       [format "%.4f" $Vbase] kN"
set Cs [expr $Vbase / ($totalMass * $g)]
puts "  Base Shear Coeff Cs:        [format "%.4f" $Cs]"
puts ""

# Save TH summary
set thFile [open "results/time_history/th_summary.txt" w]
puts $thFile "TBDY 2018 Time History Analysis Results"
puts $thFile "======================================="
puts $thFile "Model: Calibrated Stick Model (Twin Towers V9 1:50)"
puts $thFile "Ground Motion: 1999 Duzce - Bolu BOL090 (scaled 1/sqrt(50))"
puts $thFile "PGA = [format "%.4f" $pga] g"
...
close $thFile
puts ">>> Results saved to results/time_history/th_summary.txt"
puts ""
...
puts "  (OK) Roof displacement"
puts "  (OK) Base shear reaction"
puts "  (OK) All floor X-displacements"
puts ""
...
puts "  Load distribution: m_i * phi_i(mode1)"
puts ""
...
set ctrlNode $numFloors  ;# Roof node
set ctrlDOF  1           ;# X-direction
set targetDisp [expr $H_total * 0.04]  ;# 4% drift
set dU 0.01              ;# cm per step
...
puts ""
puts "  Output files:"
puts "    results/time_history/roof_disp.txt"
puts "    results/time_history/base_reaction.txt"
