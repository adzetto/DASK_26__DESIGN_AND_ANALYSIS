"""
TWIN TOWERS MODEL V8 - WEIGHTS GAP
==================================
- Geometry: 30x16cm (No Podium)
- Core Gap (1.2cm): EMPTY (Reserved for Weights) - No vertical/diagonal elements.
- Shear Walls: 3mm Balsa Panels in 3.4cm X-Bays (Floors 0-18).
- Bracing: X-Braces in upper floors and Y-direction.
- Target Weight: < 1.4kg
"""
...
print("=" * 70)
print("TWIN TOWERS MODEL V8 - WEIGHTS GAP")
print("=" * 70)

# ============================================
# GEOMETRY
# ============================================
# X Layout: 0 --(3.0)-- 3 --(8.0)-- 11 --(3.4)-- 14.4 --(1.2 GAP)-- 15.6 --(3.4)-- 19 --(8.0)-- 27 --(3.0)-- 30
X_COORDS = np.array([0.0, 3.0, 11.0, 14.4, 15.6, 19.0, 27.0, 30.0])

# Y Layout: Optimized (0, 7.4, 8.6, 16.0)
# Gap: 7.4 to 8.6 (1.2cm)
...
# CONFIGURATION
# Panels in 3.4cm bays (X-dir) up to Floor 6 (approx 7 floors) to fit < 1.4kg
PANEL_FLOOR_LIMIT = 6 
...
for tower_num, y_coords in [(1, Y_COORDS_T1), (2, Y_COORDS_T2)]:
    ...
    for floor in range(TOTAL_FLOORS):
        # 1. COLUMNS
        # OMIT columns inside the weight gap? 
        # Usually columns define the gap. 
        # We need columns at X=14.4 and X=15.6. These form the "rails" for the weights.
        # So Keep Columns.
        ...
        # 2. BEAMS (Horizontal)
        # Keep Perimeter and Core-adjacent beams.
        # Alternating Optimization for others.
        ...
        for i in range(len(X_COORDS) - 1):
            for y in y_coords:
                ...
                # Note: We KEEP beams crossing the gap to hold the tower halves together.
                # Weights usually sit ON these beams or pass between them if they are small enough?
                # User said "remain empty".
                # If weights are plates sliding on rails (columns), beams might block them.
                # BUT without beams, the tower splits.
                # Assumption: Weights are plates that fit *between* columns but *rest* on floors?
                # Or they are hung?
                # SAFEST: Keep Beams.
        ...
        if floor < TOTAL_FLOORS - 1:
            ...
            for face_idx, y in enumerate([y_coords[0], y_coords[-1]]):
                for i in range(len(X_COORDS) - 1):
                    ...
                    # CHECK 1: GAP ZONE (1.2cm) -> EMPTY
                    if abs(span_w - 1.2) < 0.1:
                        continue # NO ELEMENTS IN GAP
                        
                    # CHECK 2: NEIGHBORING BAYS (3.4cm) -> PANELS
                    # Locate: 11-14.4 and 15.6-19
                    ...
                    if is_34_bay:
                        if floor <= PANEL_FLOOR_LIMIT:
                            ...
                            # X-BRACE (Upper floors) - CHECKERBOARD (Even floors only)
                            add_element(n_bl, n_tr, 'brace_xz', tower_str, 'pin')
                            add_element(n_br, n_tl, 'brace_xz', tower_str, 'pin')
                            
                    # CHECK 3: CORNER BAYS (Outer) -> Minimal Bracing
                    # Only at key floors
                    elif floor in [0, 8, 16, 24]:
                         if (floor + i) % 2 == 0:
                             add_element(n_bl, n_tr, 'brace_xz', tower_str, 'pin')
                         else:
                             add_element(n_br, n_tl, 'brace_xz', tower_str, 'pin')
            ...
            for face_idx, x in enumerate([X_COORDS[0], X_COORDS[-1]]):
                for j in range(len(y_coords) - 1):
                    ...
                    # Y-BRACING
                    # Reduced to every 8 floors
                    if floor in [0, 8, 16, 24]:
...
# ============================================
BRIDGE_X_LEFT = 11.0
BRIDGE_X_RIGHT = 19.0 # Spans across the core 11 -> 19 (Width 8cm)
...
def create_bridge(floor_bot, floor_top, is_rigid=False):
    ...
    # STIFFENING: Horizontal X-Braces on Top and Bottom Faces of ALL bridges
    # Bottom Face
...
for fb, ft in BRIDGE_FLOORS_SINGLE:
    create_bridge(fb, ft, is_rigid=True) # Now ALL are rigid/stiffened
...
# Weight Calc
BALSA_DENSITY = 160
SECTION_FRAME = 36 # 6x6
THICKNESS_PANEL = 3

frame_types = ['column', 'beam_x', 'beam_y', 'brace_xz', 'brace_yz', 'floor_brace', 'bridge_beam', 'bridge_column', 'bridge_truss', 'bridge_rigid']
...
for _, row in panel_xz.iterrows():
    ...
    # Density 160 kg/m^3. 1 m^3 = 1e6 cm^3.
    # Weight (kg) = Vol (cm3) * Density (kg/m3) / 1e6
...
print(f"  Panels (3.4cm): {w_xz:.3f} kg (Floors 0-{PANEL_FLOOR_LIMIT})")
...
for _, elem in connectivity_df.iterrows():
    ...
    msp.add_line(tuple(coords[n1]), tuple(coords[n2]), dxfattribs={'layer': elem['element_type'], 'color': 7})
doc.saveas(EXPORTS_DIR / 'twin_towers_v8_weights.dxf')
print("DXF Saved.")
