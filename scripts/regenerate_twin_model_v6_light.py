#!/usr/bin/env python3
"""
TWIN TOWERS MODEL V6 - LIGHT DESIGN
====================================
Lightweight design:
- NO shear walls
- Minimal bracing (only at key floors)
- Essential frame structure only

Target: Reduce weight, maintain sufficient stiffness
"""
...
print("=" * 70)
print("TWIN TOWERS MODEL V6 - LIGHTWEIGHT")
print("=" * 70)
...
TOTAL_FLOORS = 26  # 26 kat = 153 cm (9 + 24*6 = 153)
...
# ============================================
# BRACING CONFIGURATION - X-BRACES AT CORE ZONES
# ============================================
# X-braces at core zones (replacing shear walls) on EVERY floor
# Corner braces only at key floors
CORNER_BRACE_FLOORS_XZ = [0, 6, 12, 18, 24]  # Corner braces
CORNER_BRACE_FLOORS_YZ = [0, 8, 16, 24]       # Corner braces
FLOOR_BRACE_FLOORS = [0, 12, 24]              # Floor diaphragm bracing
...
for tower_num, y_coords in [(1, Y_COORDS_T1), (2, Y_COORDS_T2)]:
    ...
    for floor in range(TOTAL_FLOORS):
        ...
        # =====================================================
        # BRACING - X-BRACES AT CORE ZONES (EVERY FLOOR)
        # =====================================================
        if floor < TOTAL_FLOORS - 1:

            # -------------------------------------------------
            # FRONT/BACK FACES (XZ plane)
            # Core zone (1.2cm): X-brace EVERY floor
            # Corners: only at key floors
            # -------------------------------------------------
            for face_idx, y in enumerate([y_coords[0], y_coords[-1]]):
                for i in range(len(X_COORDS) - 1):
                    ...
                    # Core zone (1.2cm panels): X-brace every floor
                    is_core = abs(panel_width - 1.2) < 0.1
                    
                    if is_core:
                        # Full X-brace (2 diagonals) at core - replaces shear wall
                        add_element(n_bl, n_tr, 'core_brace_xz', tower_str, 'pin')
                        add_element(n_br, n_tl, 'core_brace_xz', tower_str, 'pin')
                    elif floor in CORNER_BRACE_FLOORS_XZ and (i == 0 or i == len(X_COORDS) - 2):
                        # Corner braces at key floors only
            ...
            for face_idx, x in enumerate([X_COORDS[0], X_COORDS[-1]]):
                for j in range(len(y_coords) - 1):
                    ...
                    if is_core:
                        # Full X-brace (2 diagonals) at core - replaces shear wall
                        add_element(n_bl, n_tr, 'core_brace_yz', tower_str, 'pin')
                        add_element(n_br, n_tl, 'core_brace_yz', tower_str, 'pin')
            ...
            # -------------------------------------------------
            # FLOOR DIAPHRAGM BRACING - Only at key floors
            # -------------------------------------------------
...
# ============================================
# BRIDGES - Simplified connections
# ============================================
BRIDGE_X_LEFT = 11.0
BRIDGE_X_RIGHT = 19.0
...
def create_bridge(floor_bot, floor_top, is_rigid=False):
    ...
    # Extra bracing for top bridge only
    if is_rigid:
...
# Count brace elements
brace_elements = connectivity_df[connectivity_df['element_type'].str.contains('brace', case=False)]
print(f"\nBrace elements: {len(brace_elements)}")
...
LAYER_COLORS = {
    ...
    'core_brace_xz': 5, 'core_brace_yz': 5,  # Core X-braces (magenta)
    'floor_brace': 6,
...
dxf_path = EXPORTS_DIR / 'twin_towers_v6_light.dxf'
doc.saveas(dxf_path)
...
==========================
- NO shear walls (replaced with X-braces)
- Core zone X-braces at EVERY floor (1.2cm panels)
- Corner braces only at floors: {CORNER_BRACE_FLOORS_XZ} (XZ), {CORNER_BRACE_FLOORS_YZ} (YZ)
- Floor bracing only at floors: {FLOOR_BRACE_FLOORS}

Total elements: {len(connectivity_df)}
Total braces: {len(brace_elements)}
Weight: {weight:.3f} kg
""")
