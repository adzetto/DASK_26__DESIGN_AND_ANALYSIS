"""
V10e Model - Merkez YZ Braces
=============================
x=11, y=7.4 ve y=8.6 noktalarına (en yüksek deplasman) YZ çaprazları
"""
...
print("=" * 80)
print("V10e MODEL - MERKEZ YZ ÇAPRAZLARI")
print("=" * 80)
...
available_length = (1.4 - v10_weight) / (A_section * density_kg_cm3)
...
# Yüksek deplasmanlı düğümler: x=11, y=7.4 ve y=8.6
# Bunlara kat 0-1 arasında YZ çaprazı ekle

# x=11, y=7.4: node 9 (z=0) ve node 41 (z=9)
# x=11, y=8.6: node 10 (z=0) ve node 42 (z=9)
# Bunların sağ komşuları: x=14.4
# x=14.4, y=7.4: node 13 (z=0) ve node 45 (z=9)

print("\nHedef konumlar (en yüksek deplasman):")

# YZ diagonal from (11,7.4,0) to (11,8.6,9)
n1 = find_node_at(11, 7.4, 0)  # node 9
n2 = find_node_at(11, 8.6, 9)  # node 42
print(f"x=11: z=0,y=7.4 → z=9,y=8.6: {n1} → {n2}")
if n1 and n2:
    e = add_brace(n1, n2, 'center_yz')
    if e: new_elements.append(e); print(f"  Eklendi: L={e['length']:.1f}")

# Reverse: (11,8.6,0) to (11,7.4,9)
n1 = find_node_at(11, 8.6, 0)  # node 10
n2 = find_node_at(11, 7.4, 9)  # node 41
if n1 and n2:
    e = add_brace(n1, n2, 'center_yz')
    if e: new_elements.append(e); print(f"  Eklendi: L={e['length']:.1f}")

# x=14.4 de aynısını uygula
n1 = find_node_at(14.4, 7.4, 0)
n2 = find_node_at(14.4, 8.6, 9)
print(f"x=14.4: z=0,y=7.4 → z=9,y=8.6: {n1} → {n2}")
if n1 and n2:
    e = add_brace(n1, n2, 'center_yz')
    if e: new_elements.append(e); print(f"  Eklendi: L={e['length']:.1f}")

n1 = find_node_at(14.4, 8.6, 0)
n2 = find_node_at(14.4, 7.4, 9)
if n1 and n2:
    e = add_brace(n1, n2, 'center_yz')
    if e: new_elements.append(e); print(f"  Eklendi: L={e['length']:.1f}")

# x=15.6 de aynısını uygula
n1 = find_node_at(15.6, 7.4, 0)
n2 = find_node_at(15.6, 8.6, 9)
print(f"x=15.6: z=0,y=7.4 → z=9,y=8.6: {n1} → {n2}")
if n1 and n2:
    e = add_brace(n1, n2, 'center_yz')
    if e: new_elements.append(e); print(f"  Eklendi: L={e['length']:.1f}")

n1 = find_node_at(15.6, 8.6, 0)
n2 = find_node_at(15.6, 7.4, 9)
if n1 and n2:
    e = add_brace(n1, n2, 'center_yz')
    if e: new_elements.append(e); print(f"  Eklendi: L={e['length']:.1f}")

# x=19 de aynısını uygula
n1 = find_node_at(19, 7.4, 0)
n2 = find_node_at(19, 8.6, 9)
print(f"x=19: z=0,y=7.4 → z=9,y=8.6: {n1} → {n2}")
if n1 and n2:
    e = add_brace(n1, n2, 'center_yz')
    if e: new_elements.append(e); print(f"  Eklendi: L={e['length']:.1f}")

n1 = find_node_at(19, 8.6, 0)
...
# V10e oluştur
conn_v10e = pd.concat([conn_v10, pd.DataFrame(new_elements)], ignore_index=True)
v10e_weight = conn_v10e['length'].sum() * A_section * density_kg_cm3
print(f"V10e toplam ağırlık: {v10e_weight:.4f} kg")

pos.to_csv('data/twin_position_matrix_v10e.csv', index=False)
conn_v10e.to_csv('data/twin_connectivity_matrix_v10e.csv', index=False)
...
for floor in range(total_floors):
    ...
    f_elems = conn_v10e[(conn_v10e['node_i'].isin(f_nodes)) | (conn_v10e['node_j'].isin(f_nodes))]
...
print(f"""
┌────────────────────────────────────────────┐
│            V10e MODEL SONUÇLARI            │
├────────────────────────────────────────────┤
│ Ağırlık   : {v10e_weight:.4f} kg {'✓' if v10e_weight <= 1.4 else '✗'}               │
│ T1        : {T1:.4f} s                    │
...
results = pd.DataFrame({
    ...
    'Değer': ['V10e', len(conn_v10e), f'{v10e_weight:.4f}', f'{T1:.4f}', 
              f'{max_eta:.3f}', f'{max_drift:.6f}']
})
results.to_csv('results/data/structural_design_check_v10e.csv', index=False)

print("V10e ANALİZ TAMAMLANDI")
