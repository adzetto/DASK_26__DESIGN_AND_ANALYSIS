"""
TWIN TOWERS MODEL V9b - PROPERLY STIFFENED
==========================================
V9b: Keep V8 beam configuration, only ADD strategic bracing
Goal: Increase stiffness without removing beams

Key changes from V8:
1. Keep ALL original beams from V8
2. Add strategic Y-bracing at key floors
3. Extend shear walls slightly (floor 6 -> 7)
4. Remove only floor bracing weight to compensate
"""
...
print("=" * 70)
print("TWIN TOWERS MODEL V9b - PROPERLY STIFFENED")
print("=" * 70)
...
# V9b CONFIGURATION
# ============================================
PANEL_FLOOR_LIMIT = 6     # Same as V8 (keep panels at floor 6)
Y_BRACE_FLOORS = [0, 6, 12, 18, 24]  # Strategic Y-bracing at key floors (reduced)

print(f"\nV9b Configuration:")
print(f"  Shear walls up to floor: {PANEL_FLOOR_LIMIT} (was 6 in V8)")
print(f"  Y-bracing floors: {Y_BRACE_FLOORS}")
print(f"  Beams: Same as V8 (no removal)")
...
for tower_num, y_coords in [(1, Y_COORDS_T1), (2, Y_COORDS_T2)]:
    ...
    for floor in range(TOTAL_FLOORS):
        ...
        # 2. BEAMS X (SAME AS V8 - no additional removal)
        for i in range(len(X_COORDS) - 1):
            for y in y_coords:
                ...
                # V8 original: keep essential + every 2nd floor
        ...
        # 3. BEAMS Y (SAME AS V8 - no additional removal)
        for j in range(len(y_coords) - 1):
            for x in X_COORDS:
                ...
                # V8 original: keep essential + alternating with X
                if is_essential or (floor % 2 != 0):
        ...
        if floor < TOTAL_FLOORS - 1:
            ...
            for face_idx, y in enumerate([y_coords[0], y_coords[-1]]):
                for i in range(len(X_COORDS) - 1):
                    ...
                    # 3.4cm BAYS -> PANELS (V9b: extended to floor 7)
                    ...
                    # CORNER BAYS -> Key floors only (same as V8)
            ...
            # --- YZ PLANE (V9b: Enhanced Y-bracing) ---
            for face_idx, x in enumerate([X_COORDS[0], X_COORDS[-1]]):
                for j in range(len(y_coords) - 1):
                    ...
                    # V9b: Y-BRACING at strategic floors
                    if floor in Y_BRACE_FLOORS:
                        add_element(n_bl, n_tr, 'brace_yz', tower_str, 'pin')
                        add_element(n_br, n_tl, 'brace_yz', tower_str, 'pin')

            # --- FLOOR BRACING (Reduced for weight) ---
            # V9b: Only at ground, mid, and top
            if floor in [0, 12, 24]:
                for i in range(len(X_COORDS)-1):
                    for j in range(len(y_coords)-1):
                        x1, x2 = X_COORDS[i], X_COORDS[i+1]
...
doc.saveas(EXPORTS_DIR / 'twin_towers_v9b_stiffened.dxf')

print(f"\nSaved: data/twin_position_matrix_v9.csv")
print(f"Saved: data/twin_connectivity_matrix_v9.csv")
print(f"Saved: exports/twin_towers_v9b_stiffened.dxf")
print("\n" + "=" * 70)
print("MODEL V9b GENERATION COMPLETE")
print("=" * 70)
