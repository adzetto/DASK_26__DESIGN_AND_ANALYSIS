...
Proje, İstanbul Finans Merkezi yerleşkesinde ($41.002136^\circ$, $29.106832^\circ$) konumlandırılmış, iki kuleden ve kuleleri 6., 12., 18. ve 25.~katlarda bağlayan gökyüzü köprülerinden oluşan bir ikiz kule sistemidir. Yarışma kapsamında üretilecek fiziksel model balsa çıta ve balsa levhalardan imal edilecektir. Model, $40 \times 40\,\mathrm{cm}$ ahşap tabla üzerinde, her biri $30 \times 16\,\mathrm{cm}$ ($L_x \times L_y$) taban alanına sahip iki kule ve aralarında $8\,\mathrm{cm}$ köprü açıklığı olacak şekilde düzenlenmiştir. Kule yüksekliği $153\,\mathrm{cm}$'dir. Kat tanımı yarışma yönergesine uygun olarak zemin kat $9\,\mathrm{cm}$, diğer katlar $6\,\mathrm{cm}$ yüksekliğindedir.

Zemin koşulları ZC--ZD aralığında değerlendirilmiş; emniyetli tarafta kalmak üzere ZD zemin sınıfı esas alınmıştır. Deprem tehlikesi TBDY~2018'e göre DD-2 düzeyi parametreleri ile tanımlanmış; $S_{DS} = 1.008g$ ve $S_{D1} = 0.514g$ kullanılmıştır. Yapı kullanım amacı ofis/işyeri kapsamında $BKS = 3$ olup, buna bağlı Bina Önem Katsayısı $I = 1.0$ alınmıştır; bu kabullerle yapı $DTS = 1$ ve $BYS = 1$ kapsamında ele alınmıştır.

Taşıyıcı sistemin temel hedefi; deprem etkileri altında yeterli yanal rijitlik ve kontrollü enerji tüketimi sağlayarak göreli kat ötelemelerini sınırlamak, burulma etkilerini kontrol etmek ve iç kuvvetleri temele sürekli bir yük yolu ile aktarmaktır. Her kulede yanal dayanım ve rijitlik sağlayan ana sistem, moment taşıyan çerçeveler ile çapraz elemanların bir arada kullanıldığı çerçeve-çapraz hibrit sistem olarak kurgulanmıştır. Çapraz elemanlar $XZ$ ve $YZ$ düzlemlerinde stratejik noktalara yerleştirilmiş; köprü bağlantısı iki kule arasında yük paylaşımını sağlayarak global sistem rijitliğini artırmıştır. Köprü seviyeleri, iki kulenin göreli hareketini azaltan ve kuleler arasında kuvvet aktarımı oluşturan kritik davranış bölgeleri olarak değerlendirilmiştir. Köprü--kule birleşimleri modelde rijit kabul edilerek köprülerin yatay yükler altında iki kuleyi birlikte çalıştırma etkisi temsil edilmiştir.

Deprem performansı değerlendirmesinde \textsc{OpenSees}\textsuperscript{\textregistered}~\cite{opensees} ile zaman tanım alanında hesap (ZTAH) yürütülmüştür. Taşıyıcı sistem 1680~düğüm ve 4248~\texttt{elasticBeamColumn} elemanı ile idealize edilmiştir. Yer hareketi verileri olarak 1999 Düzce depremi (BOL090) kaydı ile yarışma kapsamında verilen üç kayıt (KYH-1, KYH-2, KYH-3) kullanılmıştır. SAP2000 modeli ana analizlerin yerine geçmeyecek şekilde, geometri/rijitlik tutarlılığı ve genel davranış kontrolü amacıyla doğrulama için kullanılmıştır.

Modeldeki kat kütlesi, 2026 ön şartnamesine ve geçmiş yarışmalardaki uygulamalara uygun biçimde yerleştirilen çelik tijler ve tepe plakasından oluşmaktadır. Katlar arasında düşeyde her $18\,\mathrm{cm}$'de bir olacak şekilde toplam 8~adet tij yerleştirilmiş olup her birinin ortalama kütlesi $1.6\,\mathrm{kg}$'dır. En üstte ivmeölçerin de bulunduğu, toplam kütlesi $2.2\,\mathrm{kg}$ olan bir tepe plakası yer almaktadır. Bu kütle dağılımı, ikiz kule--köprü etkileşiminin sarsma masası üzerinde gözlemlenebilir olmasını sağlayacak şekilde düzenlenmiştir.

Sonuç olarak, İstanbul Finans Merkezi'nde yüksek sismik tehlikeye karşı güvenli ve ekonomik bir yapı tasarlanması amacıyla detaylı deprem analizleri gerçekleştirilmiştir. Yapının düzenli geometrisi, düşük eksantrisite değerleri ($e_x/L_x = 0.005$, $e_y/L_y = 0.034$) ve çerçeve-çapraz hibrit taşıyıcı sistem tasarımı ile sismik performans optimize edilmiştir~\cite{tbdy2018}.
