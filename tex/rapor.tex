...
Salt moment çerçevesi sünek olmakla birlikte yanal ötelenmeyi yeterince sınırlayamamakta; münferiden çaprazlı sistem ise yüksek rijitlik sağlamakla beraber gevrek kırılmaya açık bulunmaktadır.
Bu iki uç arasındaki dengeyi kurmak üzere \textit{çerçeve--çapraz hibrit} sistem tercih edilmiştir~\cite{nehrp2009}: çapraz elemanlar eksenel kuvvet mekanizmasıyla yatay yükleri doğrudan temele ileterek yanal ötelenmeyi kontrol altına almakta; moment çerçevesi ise düğüm noktalarının dönme kapasitesi sayesinde enerji yutma ve plastik yeniden dağılım imkânı sunmaktadır.
İkiz kule konfigürasyonu benimsenerek yapı yüksekliğinin ${\sim}H/4$, $H/2$, $3H/4$ ve tepe kotlarına yerleştirilen dört köprü vasıtasıyla kuleler kuplajlanmış; bu sayede global rijitlik artırılmış, diferansiyel ötelenme ve burulma talebi asgariye indirilmiştir~\cite{patoliya2023, tambunan2024}.
...
Düğüm noktalarında kolon--kiriş birleşimi rijit, çapraz uçları ise mafsallı kabul edilmiş; böylece çaprazlar yalnızca eksenel kuvvet taşımakta, eğilme momenti çerçeve elemanlarına aktarılmaktadır.
Güçlü kolon--zayıf kiriş ilkesi gözetilerek plastik mafsalların kiriş uçlarında oluşması hedeflenmiş, kat mekanizması ihtimali bertaraf edilmiştir.
...
Her iki doğrultuda tam simetri tesis edilerek kütle--rijitlik eksantrisitesi sıfırlanmış; burulma düzensizliği kontrolü sağlanmıştır~\cite{teblig2024}.

\noindent\textit{Sistem geometrisi ve çapraz yerleşim stratejisi.}
Model $40\!\times\!40\,\mathrm{cm}$ tabla üzerinde iki simetrik kuleden oluşmaktadır (Şekil~\ref{fig:sistem}).
Her kule $30\!\times\!16\,\mathrm{cm}$ plan boyutunda, $7\!\times\!3$ düşey kolon ağı üzerine inşa edilmiş olup zemin kat yüksekliği $h_0\!=\!9\,\mathrm{cm}$, sonraki 24~kat $h\!=\!6\,\mathrm{cm}$; toplam yükseklik $H\!=\!153\,\mathrm{cm}$'dir.
Kuleler arası açıklık $8\,\mathrm{cm}$ olup köprüler 6., 12., 18.\ ve 25.~katlarda konumlandırılmıştır.
$X$ doğrultusunda köşe açıklıkları ($2{,}7\,\mathrm{cm}$) kısa tutularak çerçeve rijitliği takviye edilmiş; $Y$ doğrultusunda simetrik iki açıklık ($7{,}7\,\mathrm{cm}$) ile rijitlik dağılımı dengelenmiştir.

Çapraz elemanlar, deprem kesme kuvvetinin en yoğun olduğu alt katlara konsantre edilmiştir (Şekil~\ref{fig:sistem}).
$XZ$ düzleminde iç açıklıklarda zemin--13.~kat arasında X-çapraz çifti uygulanarak kesme kuvveti basınç ve çekme bileşenlerine ayrıştırılmış, tek yönlü burkulma riski azaltılmıştır.
...
$YZ$ düzleminde damalı (staggered) çapraz konfigürasyonu uygulanmış; ardışık katlarda çaprazlar sağ ve sol açıklıklara dönüşümlü yerleştirilerek gerilme akışı düzgün biçimde yayılmış ve tek açıklıkta aşırı gerilme yığılması önlenmiştir.
Son iki katta ise her iki düzlemde tam çapraz uygulanarak tepe katlarının lateral stabilitesi güvence altına alınmıştır.
...
% --- Kolonlar Kule 1 (siyah) ---
\foreach \y in {0.3,8,15.7} {
  \draw[black, line width=0.4pt] (\y,0) -- (\y,153);
}
% --- Kolonlar Kule 2 (siyah) ---
\foreach \y in {24.3,32,39.7} {
  \draw[black, line width=0.4pt] (\y,0) -- (\y,153);
}
% --- Kirişler beam_y: her kat (koyu gri) ---
\draw[black!70, line width=0.2pt] (0.3,0)--(15.7,0);
\draw[black!70, line width=0.2pt] (24.3,0)--(39.7,0);
\draw[black!70, line width=0.2pt] (0.3,9)--(15.7,9);
\draw[black!70, line width=0.2pt] (24.3,9)--(39.7,9);
\foreach \f in {2,...,25} {
  \pgfmathsetmacro{\z}{9+(\f-1)*6}
  \draw[black!70, line width=0.2pt] (0.3,\z)--(15.7,\z);
  \draw[black!70, line width=0.2pt] (24.3,\z)--(39.7,\z);
}
% --- Çapraz YZ: DAMALI, simetrik, alt/üst full (açık gri) ---
% FULL: alt (0→9) ve üst (135→153)
\foreach \za/\zb in {0/9,135/153} {
  \foreach \ya/\yb in {0.3/8,8/15.7,24.3/32,32/39.7} {
    \draw[black!40, line width=0.2pt] (\ya,\za)--(\yb,\zb);
    \draw[black!40, line width=0.2pt] (\yb,\za)--(\ya,\zb);
  }
}
...
% --- Düğüm noktaları Kule 1 (beyaz dolu daire) ---
\foreach \y in {0.3,8,15.7} {
...
% --- Düğüm noktaları Kule 2 (beyaz dolu daire) ---
\foreach \y in {24.3,32,39.7} {
  \fill[white] (\y,0) circle (0.8); \draw[black, line width=0.15pt] (\y,0) circle (0.8);
  \fill[white] (\y,9) circle (0.8); \draw[black, line width=0.15pt] (\y,9) circle (0.8);
  \foreach \f in {2,...,25} {
...
% --- Köprü düğüm noktaları (beyaz dolu daire) ---
...
% Alt başlık
\node[font=\tiny,anchor=north] at (20,-7) {(b) Yan görünüm ($YZ$)};
\end{scope}
\end{tikzpicture}%
}
\caption{V10 yapısal sistem: (a)~ön görünüm ($XZ$)---iç açıklıklarda X-çapraz (0--12.~kat), dış açıklıklarda 3~katlık zikzak; (b)~yan görünüm ($YZ$)---köprü bağlantıları ve 3~katlık zikzak çaprazlar. Siyah: kolon, koyu gri: kiriş/köprü, açık gri: çapraz. Beyaz daireler: düğüm noktaları. Ölçüler cm.}
\label{fig:sistem}
\end{figure}
