...
\begin{titlepage}
    ...
    \resizebox{6cm}{!}{\input{dask_tikz_logo}} \\[1cm] 
    
    {\Huge \textbf{2026}} \\[0.5cm]
    {\huge \textbf{İZMİR YÜKSEK TEKNOLOJİ ENSTİTÜSÜ PROJE TEKLİFİ}} \\[1.5cm]
    ...
    \begin{flushleft}
        ...
        2. Muhammet YAĞCIOĞLU \\[0.4cm]
        3. Yunus Emre ÇETİN \\[0.4cm]
        4. Zehra ÖZER \\[0.4cm]
        5. Ali ÖZUYSAL \\[1cm]
        \textbf{Akademik Danışman:} Dr. Gürsoy TURAN
    \end{flushleft}
...
Yüzyıllardır farklı medeniyetlere ev sahipliği yapmış, dünyanın sayılı metropollerinden biri olan İstanbul; Asya ve Avrupa arasında köprü görevi gören, tarihi dokusuyla Anadolu’nun kadim mirasını ve Avrupa’nın dinamik ruhunu birleştirerek uyumlu bir bütün oluşturan ülkemizin en önemli şehirlerinden biridir. Ayrıca, Türkiye İstatistik Kurumu verilerine göre ülkemizin hem ekonomik faaliyetleri hem de nüfusu açısından İstanbul gözle görülür bir öneme sahiptir. Bu bağlamda, İstanbul’un Ümraniye ilçesinde bulunan İstanbul Finans Merkezi’nde planlanan GÜRSOY Towers projesi; İstanbul’un küresel finans merkezi olma yolunda bölgede artan nitelikli ofis ve ticari alan ihtiyacına yanıt verirken ziyaretçilerine/çalışanlarına sürdürülebilir ve erişilebilir bir yaşam alanı sunmayı amaçlamaktadır. Projenin başlıca misyonları; bölgenin tarihsel dokusunu modern yaklaşımlarla harmanlamak, çevreci ve teknoloji odaklı sürdürülebilir yaklaşımları verimlilik esaslı mimari ve mühendislik çözümleriyle birleştirmektir.

Kuleler; yapının ağırlığı, rijitliği, maliyeti, kira getirisi ve kullanım verimliliği gibi parametreler göz önünde bulundurularak tasarlanmıştır. İki simetrik 25 katlı kuleden oluşan projede, her bir kulede birbirinden bağımsız bir çekirdek bulunmaktadır. Kuleler, 6., 12., 18. ve 24./25. katlarda yer alan köprüler ile birbirine bağlanmış; böylece yatay yükler altında birlikte çalışmaları sağlanarak göreli kat ötelenmelerinin ve burulma etkilerinin azaltılması amaçlanmıştır. 

Zemin kat, çalışanların ve ziyaretçilerin ihtiyaçlarına yönelik danışma, satış/sergi, konferans ve spor alanları gibi kamusal kullanımlara ayrılmıştır. Zemin katın devamında yer alan 1. ve 2. katlar, restoran ve mağazalara yönelik kiralanabilir alanlar olarak planlanmıştır. Orta katlarda, açık ofis düzeninde yüksek yoğunluklu kullanıma uygun çalışma alanları öngörülmektedir. Üst katlarda ise daha düşük yoğunluklu, tek kişilik veya üst düzey yöneticilere yönelik ofis düzeni planlanmıştır. Köprü katlarında, İstanbul manzarasına açılan lüks restoranlar ile sosyal ve dinlenme alanlarının oluşturulması hedeflenmiştir. Son olarak, B Kulesi’nde ikinci köprü seviyesinden itibaren rezidanslar ile 5 yıldızlı otel ve konaklama katları planlanmıştır. Bu sayede finans merkezi kullanıcıları için çalışma, yaşam ve konaklama bütünlüğünün sağlanması amaçlanmıştır.
...
Proje, İstanbul Finans Merkezi yerleşkesinde ($41.002^\circ$\,K, $29.107^\circ$\,D) konumlandırılmış ikiz kule sistemidir. Fiziksel model, $40\times40\,\mathrm{cm}$ ahşap tabla üzerinde balsa çıta ($6\times6\,\mathrm{mm}$) ve levhadan ($3\,\mathrm{mm}$) imal edilmektedir. Her kule $30\times16\,\mathrm{cm}$ ($L_x \times L_y$) plan alanı ve $8\times4$ kolon gridine sahip olup kuleler arası $8\,\mathrm{cm}$ köprü açıklığı bulunmaktadır. Zemin kat yüksekliği $9\,\mathrm{cm}$, üst 24~kat $6\,\mathrm{cm}$ olup toplam kule yüksekliği $H=153\,\mathrm{cm}$'dir (Şekil~\ref{fig:sistem}).

Sismik tehlike TBDY~2018 DD-2 düzeyi ($S_{DS}=1.008$\,g, $S_{D1}=0.514$\,g) ve ZD zemin sınıfı ($F_S=1.149$, $F_1=2.114$) ile tanımlanmıştır~\cite{afad2024}. Köşe periyotları $T_A=0.102$~s, $T_B=0.510$~s olup yapı $DTS=1$, $BYS=1$, $BKS=3$, $I=1.0$ kapsamındadır~\cite{tbdy2018}.

Taşıyıcı sistem, moment çerçeveleri ile çapraz ve panel eleman\-ların birlikte çalıştığı çerçeve--çapraz hibrit yapıdır. Alt bölgede (0--7.~katlar) iç kolon aralıklarında $3\,\mathrm{mm}$ balsa kesme panelleri uygulanarak perde benzeri rijitlik sağlanmış; üst katlarda çapraz elemanlar $XZ$~düzleminde her~4, $YZ$~düzleminde her~6~katta devam ettirilmiştir. Kuleleri bağlayan gökyüzü köprüleri yapı yüksekliğinin $\sim$$H/4$, $H/2$, $3H/4$ ve tepe seviyelerine (6., 12., 18. ve 25.~katlar) konumlandırılmıştır~\cite{patoliya2023}. Her köprü 10~kiriş, 2~kolon, 8~kafes çaprazı ve 4~rijit diyafram elemanından oluşmaktadır. Köprü--kule birleşimleri rijit kabul edilerek kulelerin yatay yükler altında birlikte çalışması sağlanmıştır.

Taşıyıcı sistem \textsc{OpenSees}\textsuperscript{\textregistered}~\cite{opensees} ile 1680~düğüm ve 4248~\texttt{elasticBeamColumn} elemanı olarak modellenmiştir. Malzeme: balsa ahşap, kalibre modülü $E=170\,\mathrm{kN/cm^2}$, $\rho=160\,\mathrm{kg/m^3}$; çıta kesiti $A=0.36\,\mathrm{cm^2}$, $I_z=I_y=0.0108\,\mathrm{cm^4}$. Yapısal ağırlık $1.076\,\mathrm{kg}$ ($<1.400\,\mathrm{kg}$ sınırı). Kat kütleleri 3., 6., \ldots, 24.~katlara $1.60\,\mathrm{kg}$ çelik tij, 25.~kata $2.22\,\mathrm{kg}$ tepe plakası olarak yerleştirilmiştir. Yer hareketi olarak 1999 Düzce (BOL090) ve yarışma kayıtları (KYH-1/2/3) kullanılmış; SAP2000 ile geometri tutarlılığı doğrulanmıştır.

Prototip ölçeğinde (1:50, $h=76.5\,\mathrm{m}$) rüzgar etkisi TS~EN~1991-1-4'e göre değerlendirilmiştir~\cite{tsen1991}. İstanbul temel rüzgar hızı $V_{b}=28.0\,\mathrm{m/s}$, Arazi~Kat.\,III ($z_0=0.3\,\mathrm{m}$) için bina tepe kotunda ortalama rüzgar hızı $v_m(76.5)=33.3\,\mathrm{m/s}$ ve hız basıncı $q_p=1.58\,\mathrm{kPa}$ ($I_v=0.18$) hesaplanmıştır. Rüzgar taban kesme kuvveti $F_{w,x}\approx c_f\,q_p\,A_{ref} = 1.3 \times 1.58 \times 1530 \approx 3.1\,\mathrm{MN}$ olup prototip deprem taban kesmesi $V_t\approx6.6\,\mathrm{MN}$'dur; sismik yükler ${\sim}2.1$~kat belirleyicidir.

Modal analiz sonuçları: $T_1=0.200$~s, $T_2=0.147$~s, $T_3=0.135$~s; üçü de plato bölgesinde ($T_A<T_n<T_B$) $S_{ae}=1.008$~g almaktadır. Göreli kat ötelemesi $\delta_i/h_i=0.00017 \ll 0.008$ olup TBDY~2018 sınırı fazlasıyla sağlanmıştır. Kütle--rijitlik eksantrisitesi $e_x/L_x=0.005$, $e_y/L_y=0.034$ ile yapı simetrisi korunmuştur~\cite{tbdy2018}.

% --- Şekil: Yapısal sistem şeması ---
\begin{figure}[H]
\centering
\resizebox{0.88\textwidth}{!}{%
\begin{tikzpicture}[>=stealth, font=\scriptsize]
% ============ (a) KESİT GÖRÜNÜM (YZ düzlemi) ============
\begin{scope}[shift={(0,0)}]
% Ground
\fill[black!6] (-0.5,-0.35) rectangle (3.95,0);
\draw[thick] (-0.5,0) -- (3.95,0);
% Tower 1 (y=0→16 → 0→1.44,  z=0→153 → 0→6.0)
\draw[thick] (0,0) rectangle (1.44,6.0);
% Tower 2 (y=24→40 → 2.16→3.60)
\draw[thick] (2.16,0) rectangle (3.60,6.0);
% Panel zone fill (floors 0-7, z=0→1.76)
\fill[black!10] (0.50,0) rectangle (0.94,1.76);
\fill[black!10] (2.66,0) rectangle (3.10,1.76);
% Bridge 1 (floors 5–6, z=33–39 → 1.29–1.53)
\fill[black!12] (1.44,1.29) rectangle (2.16,1.53);
\draw (1.44,1.29) -- (2.16,1.29); \draw (1.44,1.53) -- (2.16,1.53);
\draw[thin, black!40] (1.44,1.29) -- (2.16,1.53);
\draw[thin, black!40] (1.44,1.53) -- (2.16,1.29);
% Bridge 2 (floors 11–12, z=69–75 → 2.71–2.94)
\fill[black!12] (1.44,2.71) rectangle (2.16,2.94);
\draw (1.44,2.71) -- (2.16,2.71); \draw (1.44,2.94) -- (2.16,2.94);
\draw[thin, black!40] (1.44,2.71) -- (2.16,2.94);
\draw[thin, black!40] (1.44,2.94) -- (2.16,2.71);
% Bridge 3 (floors 17–18, z=105–111 → 4.12–4.35)
\fill[black!12] (1.44,4.12) rectangle (2.16,4.35);
\draw (1.44,4.12) -- (2.16,4.12); \draw (1.44,4.35) -- (2.16,4.35);
\draw[thin, black!40] (1.44,4.12) -- (2.16,4.35);
\draw[thin, black!40] (1.44,4.35) -- (2.16,4.12);
% Bridge 4 (floors 23–25, z=141–153 → 5.53–6.00, double)
\fill[black!12] (1.44,5.53) rectangle (2.16,6.00);
\draw (1.44,5.53) -- (2.16,5.53);
\pgfmathsetmacro{\zmid}{(5.53+6.00)/2}
\draw[thin, black!15] (1.44,\zmid) -- (2.16,\zmid);
\draw[thin, black!40] (1.44,5.53) -- (2.16,\zmid);
\draw[thin, black!40] (1.44,\zmid) -- (2.16,5.53);
\draw[thin, black!40] (1.44,\zmid) -- (2.16,6.00);
\draw[thin, black!40] (1.44,6.00) -- (2.16,\zmid);
% Dimension: total height
\draw[<->, thin] (-0.3,0) -- node[left, font=\tiny, rotate=90, anchor=south] {$H\!=\!153$\,cm} (-0.3,6.0);
% Tower widths
\draw[<->, thin] (0,-0.2) -- node[below, font=\tiny] {$16$} (1.44,-0.2);
\draw[<->, thin] (1.44,-0.2) -- node[below, font=\tiny] {$8$} (2.16,-0.2);
\draw[<->, thin] (2.16,-0.2) -- node[below, font=\tiny] {$16$} (3.60,-0.2);
% Floor heights (right side)
\draw[<->, thin] (3.75,0) -- node[right, font=\tiny] {$9$} (3.75,0.35);
\draw[<->, thin] (3.75,0.35) -- node[right, font=\tiny] {$6$} (3.75,0.59);
% Tower labels
\node[font=\tiny, black!50] at (0.72,3.0) {Kule\,1};
\node[font=\tiny, black!50] at (2.88,3.0) {Kule\,2};
% Bridge labels
\node[font=\tiny, anchor=west, black!60] at (2.20,1.41) {K.\,6};
\node[font=\tiny, anchor=west, black!60] at (2.20,2.83) {K.\,12};
\node[font=\tiny, anchor=west, black!60] at (2.20,4.24) {K.\,18};
\node[font=\tiny, anchor=west, black!60] at (2.20,5.77) {K.\,25};
% Panel zone label
\draw[decorate, decoration={brace, amplitude=2pt, mirror}, thin, black!40]
  (-0.1,0) -- node[left, font=\tiny, xshift=-2pt, black!40] {panel} (-0.1,1.76);
% Subtitle
\node[font=\footnotesize, anchor=north] at (1.80,-0.55) {(a) Kesit ($YZ$)};
\end{scope}
% ============ (b) KAT PLANI (XY düzlemi) ============
\begin{scope}[shift={(5.5,0.8)}, scale=0.08]
% Tower 1 outline
\draw[thick] (0,0) rectangle (30,16);
% Tower 2 outline
\draw[thick] (0,24) rectangle (30,40);
% Bridge zone
\fill[black!5] (10.5,16) rectangle (19.5,24);
\draw[dashed, black!40] (10.5,16) -- (10.5,24);
\draw[dashed, black!40] (19.5,16) -- (19.5,24);
% Core panel locations (thick lines on front/back faces)
\foreach \ya/\yb in {0/0, 16/16, 24/24, 40/40} {
  \draw[line width=1.8pt, black!45] (10.5,\ya) -- (13.5,\ya);
  \draw[line width=1.8pt, black!45] (16.5,\ya) -- (19.5,\ya);
}
% Column grid -- Tower 1
\foreach \x in {0, 3, 10.5, 13.5, 16.5, 19.5, 27, 30} {
  \foreach \y in {0, 7.0, 9.0, 16} { \fill[black] (\x,\y) circle (0.8); }
}
% Column grid -- Tower 2
\foreach \x in {0, 3, 10.5, 13.5, 16.5, 19.5, 27, 30} {
  \foreach \y in {24, 31.0, 33.0, 40} { \fill[black] (\x,\y) circle (0.8); }
}
% X dimension
\draw[<->, thin] (0,-3) -- node[below, font=\tiny] {$L_x\!=\!30$} (30,-3);
% Y dimensions
\draw[<->, thin] (34,0) -- node[right, font=\tiny, rotate=90, anchor=south] {$16$} (34,16);
\draw[<->, thin] (34,16) -- node[right, font=\tiny, rotate=90, anchor=south] {$8$} (34,24);
\draw[<->, thin] (34,24) -- node[right, font=\tiny, rotate=90, anchor=south] {$16$} (34,40);
% Labels
\node[font=\tiny, black!50] at (15,8) {Kule\,1};
\node[font=\tiny, black!50] at (15,32) {Kule\,2};
\node[font=\tiny, black!30] at (15,20) {köprü};
% Subtitle
\node[font=\footnotesize, anchor=north] at (15,-6) {(b) Plan ($XY$)};
\end{scope}
\end{tikzpicture}%
}
\caption{V9 yapısal sistem şeması: (a)~ikiz kule kesit görünümü ($YZ$~düzlemi)---köprü bağlantıları ve alt bölge kesme panelleri; (b)~$8\times4$ kolon gridi, çekirdek panel konumları (kalın çizgi) ve köprü açıklığı ($XY$~düzlemi). Ölçüler cm cinsindendir.}
\label{fig:sistem}
\end{figure}
...
\textbf{Cevap:} Yapının ilk üç doğal titreşim periyodu ($T_1=0.200$~s, $T_2=0.147$~s, $T_3=0.135$~s) TBDY~2018 DD-2 tasarım spektrumunun sabit ivme (plato) bölgesine ($T_A=0.102$~s $< T_n <$ $T_B=0.510$~s) düşmektedir. Bu durum, yapının tasarım depreminden alabileceği maksimum spektral ivmeye ($S_{ae}=1.008$~g) maruz kaldığı anlamına gelir; ancak periyotların ZD zemin hakim periyodundan ($T_g\approx0.4$--$0.7$~s) belirgin şekilde kısa olması rezonans riskini ortadan kaldırmaktadır.
...
% Kolonlar
\foreach \y in {0,7.0,9.0,16,24,31.0,33.0,40} {
    \draw[fill=white, draw=black, line width=0.4pt] (\y-0.4,0) rectangle (\y+0.4,\kh);
}
% X-brace çaprazlar
\draw[line width=0.3pt, gray] (0,0) -- (7.0,\kh);
\draw[line width=0.3pt, gray] (7.0,0) -- (0,\kh);
\draw[line width=0.3pt, gray] (9.0,0) -- (16,\kh);
\draw[line width=0.3pt, gray] (16,0) -- (9.0,\kh);
\draw[line width=0.3pt, gray] (24,0) -- (31.0,\kh);
\draw[line width=0.3pt, gray] (31.0,0) -- (24,\kh);
\draw[line width=0.3pt, gray] (33.0,0) -- (40,\kh);
\draw[line width=0.3pt, gray] (40,0) -- (33.0,\kh);
...
\foreach \y in {0,7.0,9.0,16} {
    \draw[line width=0.5pt] (0,\y) -- (30,\y);
}
% Kirişler X yönü (Kule 2)
\foreach \y in {24,31.0,33.0,40} {
    \draw[line width=0.5pt] (0,\y) -- (30,\y);
}
...
\draw[line width=0.3pt, gray] (0,0) -- (3,7.0);
\draw[line width=0.3pt, gray] (3,0) -- (0,7.0);
\draw[line width=0.3pt, gray] (27,0) -- (30,7.0);
\draw[line width=0.3pt, gray] (30,0) -- (27,7.0);
\draw[line width=0.3pt, gray] (0,9.0) -- (3,16);
\draw[line width=0.3pt, gray] (3,9.0) -- (0,16);
\draw[line width=0.3pt, gray] (27,9.0) -- (30,16);
\draw[line width=0.3pt, gray] (30,9.0) -- (27,16);
% Merkez X-brace
\draw[line width=0.3pt, gray] (10.5,0) -- (19.5,7.0);
\draw[line width=0.3pt, gray] (19.5,0) -- (10.5,7.0);
\draw[line width=0.3pt, gray] (10.5,9.0) -- (19.5,16);
\draw[line width=0.3pt, gray] (19.5,9.0) -- (10.5,16);

% X-Brace döşeme (Kule 2)
\draw[line width=0.3pt, gray] (0,24) -- (3,31.0);
\draw[line width=0.3pt, gray] (3,24) -- (0,31.0);
\draw[line width=0.3pt, gray] (27,24) -- (30,31.0);
\draw[line width=0.3pt, gray] (30,24) -- (27,31.0);
\draw[line width=0.3pt, gray] (0,33.0) -- (3,40);
\draw[line width=0.3pt, gray] (3,33.0) -- (0,40);
\draw[line width=0.3pt, gray] (27,33.0) -- (30,40);
\draw[line width=0.3pt, gray] (30,33.0) -- (27,40);
% Merkez X-brace
\draw[line width=0.3pt, gray] (10.5,24) -- (19.5,31.0);
\draw[line width=0.3pt, gray] (19.5,24) -- (10.5,31.0);
\draw[line width=0.3pt, gray] (10.5,33.0) -- (19.5,40);
\draw[line width=0.3pt, gray] (19.5,33.0) -- (10.5,40);
...
% KÖPRÜ - sadece x=10.5 ile x=19.5 arası (9cm), y=16 ile y=24 arası (8cm)
\draw[fill=gray!30, draw=black, line width=0.8pt] (10.5,16) rectangle (19.5,24);
% Köprü X-brace
\draw[line width=0.4pt] (10.5,16) -- (19.5,24);
\draw[line width=0.4pt] (19.5,16) -- (10.5,24);
\node[font=\scriptsize, fill=white, inner sep=1pt] at (15,20) {Köprü};

% Köprü bağlantı kolonları (vurgulu)
\foreach \x in {10.5,13.5,16.5,19.5} {
...
\draw[<->, line width=0.3pt] (10.5,43) -- (19.5,43);
\node[font=\tiny] at (15,45) {9 cm};

% Koordinatlar
\node[font=\tiny, anchor=north] at (0,-1) {0};
\node[font=\tiny, anchor=north] at (10.5,-1) {10.5};
\node[font=\tiny, anchor=north] at (19.5,-1) {19.5};
