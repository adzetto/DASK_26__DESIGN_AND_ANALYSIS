...
Sönümün yapısal tepkiye etkisini nicel olarak göstermek amacıyla KYH-1 (PGA $= 0.335$~g) altında 9 farklı $\xi$ ($\%0{,}5$--$\%20$) ile tam ZTAH ($32{,}2$~s, 6446 adım, sıfır yakınsama hatası) icra edilmiştir (Şekil~\ref{fig:damping}a). $\xi = \%0{,}5$'te $u_{max} = 1{,}127$~cm iken $\xi = \%5$'te $0{,}449$~cm'e, $\xi = \%20$'de $0{,}319$~cm'e düşmüştür. Düşük sönümde ($\xi < \%3$) eğri dikleşir zira birinci mod rezonans bandına yaklaşır; yüksek sönümde ise eğri yataylaşır çünkü enerji yutma kapasitesi doyuma ulaşır. Bu durum sönüm seçiminin tasarımda kritik bir mühendislik kararı olduğunu teyit etmektedir~\cite{tbdy2018}.
